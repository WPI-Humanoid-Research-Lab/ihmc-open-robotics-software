...
public class GazeboSensorReader implements SensorReader
{
   ...
   private final YoLong delay = new YoLong("delay", registry);
   private final YoLong timeStampDelta = new YoLong("timeStampDelta", registry);   
         ...
         StateEstimatorParameters stateEstimatorParameters, RawJointSensorDataHolderMap rawJointSensorDataHolderMap, YoVariableRegistry parentRegistry)
   {
      ...
      imu.getIMUFrame().getTransformToDesiredFrame(imu.getRigidBody().getBodyFixedFrame()).getRotation(rotationMatrix);
      Quaternion temp = new Quaternion();
      rotationMatrix.transform(temp); 
      imuToParentLinkRotationOffset.set(temp.getX(), temp.getY(), temp.getZ(), temp.getS());
...
@Override
   public void read()
   {
      try
      {
         ...
         sensorProcessing.setOrientationSensorValue(imu, new Quaternion((float)orientation.x, (float)orientation.y, (float)orientation.z, (float)orientation.w));
         sensorProcessing.setLinearAccelerationSensorValue(imu, new Vector3D(linearAcceleration.getX(), linearAcceleration.getY(), linearAcceleration.getZ()));
         sensorProcessing.setAngularVelocitySensorValue(imu, new Vector3D(angularVelocity.getX(), angularVelocity.getY(), angularVelocity.getZ()));
